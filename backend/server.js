...
app.get('/api/profile/me', requireAuth, async (req, res) => {
  const user = await db.prepare('SELECT id, full_name, email, role, company, bio, avatar_url FROM users WHERE id = $1').get(req.user.id)
...
app.get('/api/ideas', requireAuth, async (req, res) => {
  if (req.user.role === 'innovator') {
    const ideas = await db.prepare('SELECT * FROM idea_requests WHERE innovator_id = $1 ORDER BY created_at DESC').all(req.user.id)
...
app.get('/api/ideas/:id', requireAuth, async (req, res) => {
  const idea = await db.prepare('SELECT * FROM idea_requests WHERE id = $1').get(req.params.id)
...
app.post('/api/ideas', requireAuth, requireRole('innovator', 'admin'), async (req, res) => {
  ...
  const result = await db.prepare(`
    ...
    ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'submitted', 0, NULL, NULL, NULL, NOW(), NOW())
  `).run(
    req.user.id,
    title,
    category,
  ...
  const idea = await db.prepare('SELECT * FROM idea_requests WHERE id = $1').get(result.lastInsertRowid)
...
app.patch('/api/ideas/:id/status', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const existing = await db.prepare('SELECT * FROM idea_requests WHERE id = $1').get(req.params.id)
  ...
  const update = await db.prepare(`
    ...
    SET status = COALESCE($1, status),
        ...
        review_summary = COALESCE($4, review_summary),
...
app.post('/api/projects/approve', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const insert = await db.prepare(`
    ...
    ) VALUES ($1, $2, $3, $4, $5, 'approved', $6, 0, $7, $8, 'live', '', $9, NOW(), NOW())
  `)
  ...
  const project = await db.prepare('SELECT * FROM projects WHERE id = $1').get(result.lastInsertRowid)

  if (idea) {
    await db.prepare(`
      UPDATE idea_requests
      SET status = 'approved', score = COALESCE(score, 0) + 5, department = COALESCE(department, 'Product & Strategy'), updated_at = NOW()
      WHERE id = $1
    `).run(idea.id)
  }
...
app.get('/api/finance/summary', requireAuth, async (req, res) => {
  const ledger = await db.prepare('SELECT * FROM financial_ledger WHERE user_id = $1 ORDER BY created_at DESC').all(req.user.id)
...
app.post('/api/finance/ledger', requireAuth, requireRole('investor', 'admin'), async (req, res) => {
  ...
  const entry = await db.prepare('SELECT * FROM financial_ledger WHERE id = $1').get(result.lastInsertRowid)
  res.status(201).json({ entry })
})

app.get('/api/projects/:id', requireAuth, async (req, res) => {
  const project = await db.prepare('SELECT * FROM projects WHERE id = $1').get(req.params.id)
  if (!project) {
...
app.post('/api/projects', requireAuth, requireRole('innovator', 'admin'), async (req, res) => {
  ...
  const insert = await db.prepare(`
    ...
    ) VALUES ($1, $2, $3, $4, $5, $6, $7, 0, $8, $9, 'live', $10, $11, NOW(), NOW())
  `)
...
app.post('/api/community/posts', requireAuth, async (req, res) => {
  ...
  const user = await db.prepare('SELECT id, full_name, role FROM users WHERE id = $1').get(req.user.id)
  if (!user) {
    return res.status(404).json({ message: 'User not found.' })
  }
  ...
  const post = await db.prepare('SELECT * FROM community_posts WHERE id = $1').get(result.lastInsertRowid)
